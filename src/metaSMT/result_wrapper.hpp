...
namespace metaSMT {
  ...
  class result_wrapper {
    ...
    struct as_integer {
      ...
      result_type operator() ( boost::dynamic_bitset<uint64_t> const & val ) const
      {
        std::cout << "operator() boost::dynamic_bitset<uint64_t> const" << std::endl;
        ...
        if( !issigned && sizeof(Integer) <= sizeof(unsigned long) ) {
          ...
          if( isSigned ) ret = static_cast<Integer>(-1) ;
          for (unsigned i = 0; i < val.size(); ++i) {
            bool isSignedTmp = isSigned ? static_cast<bool>(1) : static_cast<bool>(0);
            ret ^= static_cast<Integer>( static_cast<bool>(val[i])^isSignedTmp) << i;
          }
